...
| **Auth** | `LOCAL_AUTH_ENABLED`, `LOCAL_ADMIN_*`, `OIDC_*`, `CORS_ORIGINS`, `TRUST_PROXY`, `COOKIE_SECURE` | Yes (at least one auth method) | API, Worker | `LOCAL_ADMIN_PASSWORD=<strong>`; `TRUST_PROXY=false`; `COOKIE_SECURE` empty | Placeholder password is rejected in production; `TRUST_PROXY` only behind a proxy; `COOKIE_SECURE` empty = per-request "auto" (Secure only over HTTPS) – set true/false only to force the flag |
...
| **Login succeeds (no error) but redirects back to login page** | `COOKIE_SECURE=true` forced while the site is served over plain HTTP – the browser drops the Secure session cookie (only possible with an explicit `COOKIE_SECURE=true` or a version before BugFix-14; the current default "auto" sets the flag only over HTTPS) | Leave `COOKIE_SECURE` empty (auto) and set `TRUST_PROXY=true` behind the proxy; recreate the API: `docker compose up -d api` |
| **"Login not available" on the login page** | The browser cannot reach the API: mixed content (HTTP API URL on an HTTPS page) or CORS origin not allowed | Set `NEXT_PUBLIC_API_BASE_URL` to an HTTPS URL (or leave it empty so it is auto-detected) and include the web origin in `CORS_ORIGINS`; for a Caddy setup see the `/api` prefix example in `docs/docker-image-guide.md` §5 |
