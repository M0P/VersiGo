...
| Admin: Passwort zurücksetzen | ADMIN | `/admin/users` | Button "Passwort zurücksetzen" (nur bei User mit lokalem Login, `hasCredential`), Dialog (neues Passwort + Bestätigungsfeld), Submit | Passwort-Hash ersetzt, Audit `USER_PASSWORD_RESET`, Toast "Passwort zurückgesetzt" | `RolesGuard(@Roles(GlobalRole.ADMIN))` | Inline-Fehler "Account hat kein lokales Passwort" (409), Mismatch-Prüfung, sonst "Zurücksetzen fehlgeschlagen"; Sessions werden nicht widerrufen | Unit: `user-admin.service.spec.ts` |
...
| Passwort ändern | USER, ADMIN | `/settings` (Tab "Profil") | Formular (aktuelles Passwort, neues Passwort, Bestätigung, min. 12 Zeichen), Submit-Button | Passwort-Hash ersetzt, Audit `PASSWORD_CHANGED`, Felder geleert, Toast "Passwort geändert" | `AuthGuard` + Rate-Limit `change-password` (429) | Toast "Aktuelles Passwort ist falsch" (403), "Account hat kein lokales Passwort" (409), 401 Redirect zu `/login`, 429 "Zu viele Fehlversuche" | Unit: `auth.service.spec.ts`, `auth.controller.spec.ts` |
...
| AI-Extraktion starten | USER, ADMIN (Owner) | `/policies/[id]` (Tab "Dokumente") | Button "Mit AI extrahieren" pro Dokument (nur wenn AI_ENABLED) | Job in Queue `ai-extraction`, Status `PENDING` | `AuthGuard` + `HouseholdMembershipGuard` + Owner-Check + Capability `ai` (AI_ENABLED) | Button disabled + Tooltip "AI nicht konfiguriert" | Unit: `ai-assist.controller.spec.ts` |
| AI-Deckungszusammenfassung | USER, ADMIN (Owner) | `/policies/[id]` (Tab "Deckung") | Button "Zusammenfassung generieren" (nur wenn AI_ENABLED) | Job in Queue, Ergebnis als Markdown angezeigt | `AuthGuard` + `HouseholdMembershipGuard` + Owner-Check + Capability `ai` (AI_ENABLED) | Button disabled + Tooltip "AI nicht konfiguriert" | Unit: `ai-assist.controller.spec.ts` |
...
| Feature-Verwaltung | ADMIN | `/admin/features` | Karte pro Feature (KI, OIDC, Paperless, Storage, Family-Sharing): Master-Toggle, Inline-Editoren (Boolean/Select/Text/Secret), Reset, Connectivity-Test, Badges "Neustart erforderlich" + Quelle (UI/ENV/DEFAULT) | `PUT/DELETE/POST /admin/system-config/:key` (Katalog-Allowlist, Typvalidierung, Secret-Verschlüsselung); `runtime`-Kategorie wirkt sofort, `restart`-Kategorie erst nach Neustart | `RolesGuard(@Roles(GlobalRole.ADMIN))` | Inline-Alert "Fehler beim Laden/Speichern", ungültiger UI-Wert wird ignoriert (`uiValueInvalid`-Badge) | Smoke: Step 8b |
...
| Funktion | Zielrolle | UI-Einstiegspunkt | Bedienelemente | Erwartetes Ergebnis | Berechtigungsprüfung | Fehlerzustand | Test |
|----------|-----------|-------------------|----------------|---------------------|----------------------|---------------|------|
| Freigaben übersicht | USER, ADMIN | `/household/shares` | Tabelle aller Shares (Ziel, Scope, Berechtigung), Button "Neue Freigabe" | Liste der Shares des Households | `AuthGuard` + `HouseholdMembershipGuard` + `FamilySharingGuard` (Feature `familySharing`/`FAMILY_SHARING_ENABLED`) | Leere State "Keine Freigaben"; bei deaktiviertem Feature 403 | Unit: `family-sharing.controller.spec.ts` |
