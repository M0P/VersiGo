...
import { Input, Select } from '../ui/form-field';
...
export function AdminFeaturesSection({
  ...
  onEntryRefresh,
  onError,
}: AdminFeaturesSectionProps): ReactElement {
  ...
  const handleToggle = async (feature: (typeof FEATURES)[number], next: boolean) => {
    ...
    try {
      ...
      await onEntryRefresh(feature.toggleKey);
    } catch (e: unknown) {
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    try {
      ...
      setEditingKey(null);
      await onEntryRefresh(editingKey);
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : t('common.unknownError');
      onError(message);
    } finally {
  ...
  const handleReset = async (entry: SystemConfigEntry) => {
    ...
    try {
      ...
      await onEntryRefresh(entry.key);
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : t('common.unknownError');
      onError(message);
    }
