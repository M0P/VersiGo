# Arbeitspaket AP-06: family-sharing

## Ziel
Familienfreigaben für Verträge und Dokumente.

## Prompt für das Umsetzungsmodell

```text
Du implementierst AP-06 im Projekt VersiGo.
...
Arbeite in einem neuen Branch `feat/AP-06-family-sharing` auf Basis des aktuellen `main`. Direkte Änderungen an `main` sind verboten.

Aufgabe:
Familienfreigaben für Verträge und Dokumente.

Akzeptanzkriterien:
- Freigaben unterstützen mindestens `read` und `write`.
- Freigaben können auf Vertrag, Dokument, Kategorie oder alle eigenen Objekte begrenzt sein.
- Entzug einer Freigabe wirkt unmittelbar auf API und UI.
- Eigentümerschaft und Household-Trennung bleiben erhalten.
- Berechtigungsmatrix ist getestet.
