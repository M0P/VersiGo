# Arbeitspaket AP-02: identity-access

## Ziel
OIDC-Login, Benutzer, Haushalte, Mitgliedschaften, Rollen und Session-Schutz.

## Prompt für das Umsetzungsmodell

```text
Du implementierst AP-02 im Projekt Insura.
...
Arbeite in einem neuen Branch `feat/AP-02-identity-access` auf Basis des aktuellen `main`. Direkte Änderungen an `main` sind verboten.

Aufgabe:
OIDC-Login, Benutzer, Haushalte, Mitgliedschaften, Rollen und Session-Schutz.

Akzeptanzkriterien:
- OIDC Discovery und Claim-Mapping sind über Admin-Settings vorbereitbar.
- Mandantentrennung erfolgt mindestens auf Household-Ebene.
- Rollen `owner`, `admin`, `member`, `viewer` sind durchgesetzt.
- Nicht authentifizierte Zugriffe auf geschützte APIs und UI werden abgewiesen.
- Unit- und Integrationstests decken Rollen und Zugriffstrennung ab.
